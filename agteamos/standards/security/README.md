...
- **Continuous audit in CI**: run `npm audit` / `pip audit` / `trivy` on every pipeline run — fail the build on critical/high vulnerabilities.
- **Pin versions**: use exact versions or hashes (`sha256`) in lockfiles to prevent dependency-injection attacks (typosquatting, compromised transitive deps).
- **MFA on critical actions**: require Multi-Factor Authentication for payments and admin-level operations, on top of the JWT/OIDC flows above.
- **Threat modeling**: run STRIDE analysis before implementing any new component with a trust boundary — see the `threat-modeling` skill for the full PASTA+STRIDE+LINDDUN process and templates.
