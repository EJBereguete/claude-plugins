...
```bash
# Export to agteamos/api/openapi.json after running the server
curl http://localhost:8000/openapi.json > agteamos/api/openapi.json
```

The spec lives at `agteamos/api/openapi.json` and is regenerated as part of the CI pipeline to keep it in sync with the implementation.

---

## GraphQL — when to prefer it over REST

- Multiple clients with distinct data needs (web, mobile, partner integrations).
- Complex queries with many relations where REST would cause N+1 requests.
- REST is causing measured, real over-fetching or under-fetching.

**Do NOT reach for GraphQL when:** the API is simple, clients are homogeneous, or the team has no GraphQL operating experience — REST + the conventions above is the default.
...
| Stack | Archivo |
|-------|---------|
| Python / FastAPI | [examples/python.md](./examples/python.md) |
| C# / ASP.NET Core | [examples/csharp.md](./examples/csharp.md) |
| TypeScript / Node.js | [examples/typescript.md](./examples/typescript.md) |
