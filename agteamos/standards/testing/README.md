...
test.describe('Invoice creation flow', () => {
  ...
  test('user can create an invoice and see the confirmation', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'agteamos/changes/42-slug/evidence/create-invoice.png',
      fullPage: true,
    })
  })
  ...
  test('user sees error when submitting without a client name', async ({ page }) => {
    ...
    await page.screenshot({
      path: 'agteamos/changes/42-slug/evidence/create-invoice-validation-error.png',
      fullPage: true,
    })
  })
})
...
Consumer (frontend) defines the contract; provider (backend) verifies it in CI:

```javascript
// consumer — tests/pact/userConsumer.pact.test.js
const provider = new PactV3({ consumer: "frontend_app", provider: "user_api" });
await provider
  .given("a user with id usr_123 exists")
  .uponReceiving("a GET request for user usr_123")
  .withRequest({ method: "GET", path: "/api/v1/users/usr_123" })
  .willRespondWith({ status: 200, body: { data: { id: MatchersV3.regex("usr_[a-z0-9]+", "usr_123") } } })
  .executeTest(async (mockServer) => { /* assert against mockServer.url */ });
```

```python
# provider — tests/pact/test_user_provider.py
def test_provider_honors_consumer_contract():
    verifier = Verifier(provider="user_api", provider_base_url="http://localhost:8000")
    result = verifier.verify_with_broker(broker_url="https://pact-broker.example.com")
    assert result == 0, "Provider does not honor consumer contracts"
```

A contract break must block the merge immediately.
...
```python
# locust — basic load test
from locust import HttpUser, task, between
class UserAPILoadTest(HttpUser):
    wait_time = between(1, 3)
    @task(3)
    def get_user(self): self.client.get("/api/v1/users/usr_123")
...
# TypeScript — Stryker
npx stryker run
# Target: mutation score > 80% on domain modules
```
...
1. Unit coverage < 80% on new code → block.
2. Accessibility violations at critical/serious level → block.
3. Contract break (Pact) → block immediately.
4. Mutation score < 60% on new domain modules → block.
5. Detected visual regression → block (requires manual approval).

## Enforcement Checklist

- [ ] Every use-case function has: happy path + error + edge case tests
- [ ] Tests named with `test_{what}_{condition}_{expected}` (or equivalent per stack)
- [ ] Factories used to build test data — no hardcoded objects inline
- [ ] Mocks only for external dependencies (network, DB) — never for domain logic
- [ ] Integration tests use real containers (Testcontainers), not SQLite/in-memory fakes
- [ ] Pact contract updated whenever the API changes
- [ ] Performance test run before major releases
...
| Stack | Archivo |
|-------|---------|
| Python / pytest | [examples/python.md](./examples/python.md) |
| C# / xUnit | [examples/csharp.md](./examples/csharp.md) |
| TypeScript / Vitest | [examples/typescript.md](./examples/typescript.md) |
