...
## Safe vs Dangerous Operations (lock behavior)

**SAFE (no table lock):**
```sql
ALTER TABLE orders ADD COLUMN notes TEXT;                          -- nullable column
ALTER TABLE orders ADD COLUMN priority INTEGER DEFAULT 0;          -- constant default
CREATE INDEX CONCURRENTLY idx_orders_customer_id ON orders(customer_id);  -- never inside BEGIN/COMMIT
ALTER TABLE orders ADD CONSTRAINT fk_orders_customer_id
    FOREIGN KEY (customer_id) REFERENCES customers(id) NOT VALID;  -- validate separately, brief lock
ALTER TABLE orders VALIDATE CONSTRAINT fk_orders_customer_id;
```

**DANGEROUS (locks the table — avoid in prod on tables with data):**
```sql
ALTER TABLE users ALTER COLUMN phone SET NOT NULL;   -- BLOCKS — no default on existing rows
ALTER TABLE orders ALTER COLUMN total TYPE BIGINT;   -- BLOCKS — type change rewrites the table
CREATE INDEX idx_orders_date ON orders(created_at);  -- BLOCKS — missing CONCURRENTLY
-- DROP TABLE, DROP COLUMN — irreversible, always Contract-phase only
```

**Adding `NOT NULL` without blocking:**
```sql
...
ALTER TABLE users VALIDATE CONSTRAINT users_phone_not_null;   -- brief lock only
```

**Lock timeouts — mandatory in production:**
```sql
SET lock_timeout = '5s';       -- fail fast instead of blocking writers indefinitely
SET statement_timeout = '30s';
```

**Batch backfills on large tables** (avoids long-held locks and autovacuum pressure):
```python
# Alembic — backfill in batches
def upgrade() -> None:
    op.add_column("orders", sa.Column("display_currency", sa.String(3), nullable=True))
    connection = op.get_bind()
    batch_size, last_id = 10_000, 0
    while True:
        result = connection.execute(text("""
            UPDATE orders SET display_currency = currency
            WHERE id > :last_id AND display_currency IS NULL
            ORDER BY id LIMIT :batch_size RETURNING id
...
npx prisma migrate deploy                           # prod (applies only, never generates)

# Supabase
supabase db diff --file add_phone_to_users
supabase db push

# EF Core (.NET)
dotnet ef migrations add AddDisplayCurrencyToOrders --project src/Infrastructure
dotnet ef database update
dotnet ef migrations script                          # generate plain SQL
```
...
- [ ] Migration tested in dev and staging against a prod-like dataset
- [ ] Recent database backup confirmed
- [ ] Rollback script prepared and tested
- [ ] `EXPLAIN ANALYZE` run to estimate duration
- [ ] `lock_timeout` and `statement_timeout` set
- [ ] `CREATE INDEX` uses `CONCURRENTLY`
- [ ] `ADD COLUMN NOT NULL` has a DEFAULT or a planned batch backfill
- [ ] No `DROP TABLE`/`DROP COLUMN` without confirming nothing still references it
- [ ] Team notified if the migration can affect availability
- [ ] Active monitoring during the migration window
