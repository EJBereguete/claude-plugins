...
```
Is the state local to one component, nobody else needs it?
  → useState / useReducer

Is the state shared across a small subtree (theme, auth, preferences)?
  → React Context — WARNING: context whose value changes frequently causes mass re-renders

Is the state global app state (cart, logged-in user, notifications)?
  → Zustand (React) / Pinia (Vue) — see the Zustand section above

Does the state come FROM the server (fetching, cache, mutations)?
  → TanStack Query / SWR — NEVER put server data in Redux/Zustand.
    They handle cache, revalidation and background refetch — a plain store doesn't.
```

```typescript
// Server state — always handle all 3 states explicitly
function UserList() {
...
function CreateUserForm() {
  ...
  const onSubmit = async (data: FormData) => {
    try { await userApi.create(data) } catch (error) { handleApiError(error) }
  }
  return (
