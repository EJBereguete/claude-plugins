# Mantener los standards al día (`agteamos-standards`)

`agteamos/standards/` es donde AgTeamOS deja constancia de qué convenciones de código realmente aplica tu proyecto — no un genérico copiado del plugin, sino el resultado de leer tu código real y compararlo contra los 11 estándares base. Ver el diseño completo en [Capa de standards](../conceptos/capa-de-standards.md); esta guía es la referencia operativa de cuándo y cómo correr la skill.

## Cuándo correrla

- Automáticamente, como parte de `agteamos-onboard` la primera vez que se documenta un proyecto con código existente.
- A demanda, cuando quieras: `/agteamos-standards`.
- **Cuándo re-ejecutarla**: después de un cambio de stack relevante (nuevo framework, migración de ORM), después de resolver una desviación documentada (`status: deviates` → ya corregida), o periódicamente como parte de una auditoría (`agteamos-audit` la puede disparar si los estándares no se revisaron en mucho tiempo).
...
Se regenera junto con `standards.yml`, en el mismo paso, siempre que corre `agteamos-standards`.
...
- Correr esta skill en un repo vacío — para eso está `agteamos-new-project`/`agteamos-setup`, no `agteamos-standards`.
