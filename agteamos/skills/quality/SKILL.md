...
- **Tautological tests**: the expected value is computed with the same
  ...
  not a recomputed formula. See `agteamos-spec`'s Acceptance
...
3. For a topic `status: done`, read the matching
   ...
   `ensure-artifact(standards.<tema>)` (see `agteamos-context`
...
Follow the `agteamos-pr` skill for the PR feedback conventions (tone, severity labels, structure). Use this exact format:
...
**Próximo paso sugerido**: si quedaron Bloqueantes, volver a
`agteamos-implement`/`agteamos-build` para resolverlos; si no hay
Bloqueantes, `agteamos-implement` (merge) — ver `agteamos-context`
§Próximo paso.
...
`agteamos-knowledge` compara el proyecto contra 7 estándares base, pero es una
...
`agteamos-context` §Lazy Artifacts) antes de evaluar los smells —
...
`agteamos-knowledge` (Disparador 2): *"El hallazgo '<regla>' ya apareció 3 veces.
...
**Próximo paso sugerido**: vuelve a la skill que disparó este modo (ej.
`agteamos-implement` Step 7) — no tiene un "después" propio.
...
**Próximo paso sugerido**: no aplica — este modo corre en CI/pre-commit, no
lo dispara ni lo cierra un agente en una conversación.
...
> **Nota — no confundir con `agteamos-meta`**: este modo (auditoría
> integral) audita el codigo/arquitectura del **PROYECTO CONSUMIDOR** (el repo
> donde se instaló AgTeamOS). `agteamos-meta` es una skill distinta que
...
If neither exists, run `agteamos-knowledge` (L0, default) before proceeding — an
...
(ver `agteamos-context` §Lazy Artifacts) para esos temas
puntuales — no hace falta `agteamos-knowledge --full` para auditar, alcanza con
...
Antes de publicarlo, correr el checklist pre-envío de `agteamos-pr`
...
**Próximo paso sugerido**: `agteamos-decisions` (ADR para hallazgos
arquitectónicos grandes) o `agteamos-task` (atacar los P0 del radar) — ver
