---
name: agteamos-build-api-workflow
description: >
...
3. Read the matching `agteamos/standards/<folder>/README.md` (and `deviations.md` if
   present) for each resolved topic.
...
**If it does not exist**: continue without blocking, but record explicitly in the
mini-spec's Technical Decisions (Step 2) that "no `agteamos/standards/` found — could
not verify against project standards" so this shows up as traceable, not silently skipped.
