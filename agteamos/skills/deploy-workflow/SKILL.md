...
Execute the full `agteamos-production-readiness` skill checklist before any merge or deploy command.
...
Create `agteamos/devops/prr/PRR-v<semver>.md` as specified in the `agteamos-production-readiness` skill.
...
Execute the `agteamos-dora-metrics` skill to recalculate the sprint metrics if this is the sprint's last deployment.
