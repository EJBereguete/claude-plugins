...
Invocar `agteamos-knowledge --inject <intent|paths>` con la descripción de la
tarea y los paths que se modificarán. No leer el registry ni recorrer topics
directamente desde esta skill.

1. El output válido contiene solo paths, uno por línea.
2. Leer únicamente esos paths project-owned.
3. Si el protocolo señala un topic `pending`/`stale`, ejecutar como máximo un
   `ensure-artifact(standards.<id>)` en este step; discovery usa el `id`/folder
   canónico del registry y evidencia real del proyecto.
4. Repetir `--inject` tras discovery y aplicar solo los paths devueltos.
5. Si devuelve vacío, registrar en Technical Decisions que no había una
   convención project-owned lista y relevante; continuar sin inventarla.

Los patrones genéricos de Step 4 son ejemplos de implementación, no standards
del plugin. Una convención project-owned respaldada por evidencia prevalece.
...
Invocar `agteamos-knowledge --inject <intent|paths>` con el intent de UI y los
paths previstos. Consumir su output como una lista de paths, sin explicaciones:

1. leer únicamente los paths project-owned devueltos;
2. si el protocolo señala un topic `pending`/`stale`, generar como máximo uno
   mediante `ensure-artifact(standards.<id>)`;
3. repetir `--inject` y aplicar los paths resultantes;
4. si el output queda vacío, anotarlo al presentar el wireframe y continuar
   sin convertir ejemplos genéricos en una convención del proyecto.

No resolver keywords ni aliases manualmente aquí. El registry metadata-only,
el índice y la normalización a folders canónicos pertenecen a
`agteamos-knowledge`.
