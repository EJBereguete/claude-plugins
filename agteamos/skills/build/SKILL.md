...
3. For each resolved topic with `status: done`, read the matching
   ...
   `agteamos-context-engineering` §Lazy Artifacts) — this runs
   `agteamos-project-docs --topic <tema> --scope <paths de la tarea>` in bounded
...
`agteamos-project-docs`/`agteamos-setup` corridos todavía): continue without
...
Execute the `agteamos-pr-standards` skill to format and open the PR correctly.
...
3. For each resolved topic with `status: done`, read the matching
   ...
   `agteamos-context-engineering` §Lazy Artifacts) — generates just that
...
Execute the `agteamos-pr-standards` skill to format the PR.
